...
class Summarizer:
    def __init__(self, stopword_file="vietnamese-stopwords-dash.txt", extractive_model="Fsoft-AIC/videberta-base", 
        ...
        self.runtime = boto3.client('sagemaker-runtime', config=boto3.session.Config(max_pool_connections=15))
    ...
    def get_embeddings(self, texts: List[str]):
        """Get embeddings for multiple texts in dynamically sized even batches."""
        
        num_texts = len(texts)
        
        # Determine batch size: find the largest even number that divides num_texts well
        if num_texts <= 2:
            batch_size = num_texts  # Small input, keep batch size small
        else:
            batch_size = max(2, num_texts // 4 * 2)  # Ensure it's even
        
        embeddings = []
        for i in range(0, num_texts, batch_size):
            batch = texts[i:i + batch_size]  # Create batches of even size

            payload = {"inputs": batch}  # Send batch in a single request
            ...
            result = json.loads(response['Body'].read())  # Parse response

            for token_embeddings in result:  
                sentence_embedding = np.mean(np.array(token_embeddings), axis=0)  # Mean pooling
    ...
    def extractive_summary(self, sentences, num_sentences=3):
        ...
        sentence_embeddings = [emb.squeeze(0) for emb in sentence_embeddings]
